...
function rDebtCred(){
  // Calculate debtors (customers who owe us)
  const debtors=S.customers.map(c=>({
    ...c,
    balance:calcCustomerBalance(c.id),
    type:"customer"
  })).filter(c=>Math.abs(c.balance)>0.01);
  
  // Calculate creditors (suppliers we owe or paid)
  const creditors=S.suppliers.map(s=>({
    ...s,
    balance:calcSupplierBalance(s.id),
    type:"supplier"
  })).filter(s=>Math.abs(s.balance)>0.01);
  
  const totalDebtors=debtors.filter(d=>d.balance>0).reduce((s,d)=>s+d.balance,0);
  const totalCreditors=creditors.reduce((s,c)=>s+Math.abs(c.balance),0);
  const netPosition=totalDebtors-totalCreditors;
  
  const debtorList=debtors.length>0?debtors.map(d=>`
    <div class="list-item" onclick="showPartyStatement('customer','${d.id}')">
      <div class="list-icon" style="background:${d.balance>0?'var(--green-dim)':'var(--red-dim)'};color:${d.balance>0?'var(--green)':'var(--red)'};">👤</div>
      <div class="list-content">
        <div class="list-title">${d.name}</div>
        <div class="list-sub">${d.phone||d.email||"—"}</div>
      </div>
      <div class="list-right">
        <div class="list-amount" style="color:${d.balance>0?'var(--green)':'var(--red)'};">${d.balance>0?'+':''}${fmA(d.balance)}</div>
        <div class="list-meta">${d.balance>0?(S.lang==="fa"?"بدهکار":"owes you"):(S.lang==="fa"?"اضافه پرداختی":"overpaid")}</div>
      </div>
    </div>`).join(""):`<div class="empty"><div class="empty-icon">👥</div><p>No debtors</p></div>`;
  
  const creditorList=creditors.length>0?creditors.map(c=>`
    <div class="list-item" onclick="showPartyStatement('supplier','${c.id}')">
      <div class="list-icon" style="background:var(--amber-dim);color:var(--amber);">🏭</div>
      <div class="list-content">
        <div class="list-title">${c.name}</div>
        <div class="list-sub">${c.phone||c.email||"—"}</div>
      </div>
      <div class="list-right">
        <div class="list-amount" style="color:var(--amber);">${fmA(Math.abs(c.balance))}</div>
        <div class="list-meta">${S.lang==="fa"?"پرداخت شده":"paid to"}</div>
      </div>
    </div>`).join(""):`<div class="empty"><div class="empty-icon">🏭</div><p>No creditors</p></div>`;
  
  $("p-debtorsCreditors").innerHTML=`
    <div class="page-header"><div>
      <div class="page-title">${t("debtorsCreditors")}</div>
      <div class="page-sub">${S.lang==="fa"?"موجودی خالص":"Net Position"}: <span style="color:${netPosition>=0?'var(--green)':'var(--red)'};font-weight:700;">${fmA(netPosition)}</span></div>
    </div></div>
    
    <div class="stat-grid">
      <div class="stat-card green"><div class="sc-icon">📥</div><div class="sc-label">${t("totalOwed")}</div><div class="sc-value" style="color:var(--green);">${fmA(totalDebtors)}</div><div class="sc-sub">${debtors.filter(d=>d.balance>0).length} ${t("debtors")}</div></div>
      <div class="stat-card amber"><div class="sc-icon">📤</div><div class="sc-label">${t("totalOwing")}</div><div class="sc-value" style="color:var(--amber);">${fmA(totalCreditors)}</div><div class="sc-sub">${creditors.length} ${t("creditors")}</div></div>
    </div>
    
    <div class="card">
      <div class="card-head"><span class="card-title">📥 ${t("debtors")} (${S.lang==="fa"?"از تو طلب دارن":"customers who owe you"})</span></div>
      ${debtorList}
    </div>
    
    <div class="card">
      <div class="card-head"><span class="card-title">📤 ${t("creditors")} (${S.lang==="fa"?"بهشون پرداخت شده":"suppliers paid"})</span></div>
      ${creditorList}
    </div>`;
}
