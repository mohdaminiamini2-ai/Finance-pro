...
const AED_DENOMS=[1000,500,200,100,50,20,10,5];
...
function rDash(){
  ...
  const lowBal=S.accounts.filter(a=>a.type!=="credit"&&(a.balance||0)<S.lowBalanceThreshold);
  ...
  const alerts=[];
  if(net<S.lowBalanceThreshold)alerts.push(`<div class="alert alert-red">${t("lowFunds")} ${fmA(net)}</div>`);
  if(lowBal.length>0)alerts.push(`<div class="alert alert-amber">⚠️ ${lowBal.length} account${lowBal.length>1?"s":""} low: ${lowBal.map(a=>`<b>${a.name}</b>`).join(", ")}</div>`);
...
function rCashier(){
  ...
  const denomHTML=AED_DENOMS.map(d=>`
    <div class="denom-item">
      <div>
        <div class="denom-label">${d} AED</div>
        <div style="font-size:10px;color:var(--text3);">= ${fmN(d*(S.denominations[d]||0))}</div>
      </div>
      <input type="number" class="denom-input" id="dn-${d}" value="${S.denominations[d]||0}" min="0" oninput="updateDenomTotal()"/>
    </div>`).join("");
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="denom-total">
          ...
          ${Math.abs(denomTotal-totalCash)>0.01?`<div style="font-size:11px;color:var(--amber);margin-top:6px;">⚠️ Mismatch: ${fmA(denomTotal-totalCash)}</div>`:""}
        </div>
      </div>
    </div>
...
async function saveDenominations(){
  for(const d of AED_DENOMS){
    const count=parseInt($("dn-"+d)?.value)||0;
...
function showVoucher(type,editId){
  ...
  const vNo=v?.voucher_no||((type==="receipt"?"RV":"PV")+"-"+Date.now().toString().slice(-6));
...
async function saveVoucher(type,eid){
  ...
  const row={voucher_no:$("vno")?.value||"",type,date:$("vdate")?.value||today(),party:$("vparty")?.value||"",amount:amt,currency:$("vcur")?.value||"AED",payment_mode:$("vpm")?.value||"cash",account_id:$("vacc")?.value||null,note:$("vnote")?.value||""};
  if(eid){
    ...
    if(row.account_id){
      ...
      if(acc){
        ...
        await upd("accounts",row.account_id,{balance:newBal});
        acc.balance=newBal;
      }
    }
    const tx=await ins("transactions",{type:type==="receipt"?"income":"expense",amount:amt,currency:row.currency,account_id:row.account_id,category:type==="receipt"?"sales":"other",note:`${row.voucher_no} - ${row.party}`,date:row.date});
...
async function delVoucher(id){
  ...
  closeModal();toast(t("deleted"),"info");render(S.page);
}
...
function showVoucherDetail(id){
  ...
  modal(v.type==="receipt"?t("receiptVoucher"):t("paymentVoucher"),`
    ...
    `<button class="btn btn-danger btn-sm" onclick="delVoucher('${id}')">✕</button>
     ...
     <button class="btn btn-ghost btn-sm" onclick="closeModal();showVoucher('${v.type}','${id}')">✎</button>
...
async function saveChIn(eid){
  ...
  closeModal();toast(t("saved"),"ok");render("chequesIn");
}
...
async function delChIn(id){
  ...
  closeModal();toast(t("deleted"),"info");render("chequesIn");
}
...
function showCart(){
  ...
  modal("Cart",`<div class="card">${items}</div><div style="background:var(--glass);padding:14px;border-radius:10px;margin-top:14px;"><div style="display:flex;justify-content:space-between;font-size:18px;font-weight:700;"><span>Total</span><span style="font-family:var(--mono);color:var(--accent);">${fmA(total)}</span></div></div>`,
    `<button class="btn btn-danger btn-sm" onclick="S.cart=[];closeModal();render('pos')">Clear</button><button class="btn btn-primary" onclick="closeModal();checkout()">Checkout →</button>`);
}
function checkout(){
...
async function delAcc(id){if(!confirm(t("confirmDel")))return;await del("accounts",id);S.accounts=S.accounts.filter(x=>x.id!==id);closeModal();toast(t("deleted"),"info");render("accounts");}
...
function showTxDetail(id){
  ...
  modal("Transaction",`<div style="text-align:center;padding:20px;"><div style="font-size:32px;font-family:var(--mono);font-weight:300;color:${tx.type==="income"?"var(--green)":"var(--red)"};">${tx.type==="income"?"+":"−"}${fmA(tx.amount,tx.currency)}</div></div>
    ...
    `<button class="btn btn-danger btn-sm" onclick="closeModal();delTx('${id}')">✕</button><button class="btn btn-secondary" onclick="closeModal()">${t("close")}</button>`);
}
...
function selProd(i,sel){const o=sel.options[sel.selectedIndex];if(o.value){S.invItems[i].name=o.text;S.invItems[i].unit_price=+o.dataset.price||0;S.invItems[i].total=S.invItems[i].unit_price*S.invItems[i].qty;}rII();calcInv();}
...
function showInvDetail(id){
  ...
  modal(inv.invoice_no,`
    ...
    `<button class="btn btn-danger btn-sm" onclick="closeModal();delInv('${id}')">✕</button>
     ...
     ${inv.status==="submitted"?`<button class="btn btn-success btn-sm" onclick="closeModal();markInvPaid('${id}')">✓ ${t("paid")}</button>`:""}
...
async function delProd(id){if(!confirm(t("confirmDel")))return;await del("products",id);S.products=S.products.filter(x=>x.id!==id);closeModal();toast(t("deleted"),"info");render("inventory");}
...
function showList(type){
  ...
  const list=items.map(it=>{
    ...
    if(type==="payment"||type==="receipt"){
      return `<div class="list-item" onclick="closeModal();showVoucherDetail('${it.id}')"><div class="list-icon" style="background:${type==="receipt"?"var(--green-dim)":"var(--red-dim)"};color:${type==="receipt"?"var(--green)":"var(--red)"};">${type==="receipt"?"📥":"📤"}</div><div class="list-content"><div class="list-title">${it.party||it.voucher_no}</div><div class="list-sub">${fmD(it.date)} · ${it.payment_mode}</div></div><div class="list-right"><div class="list-amount" style="color:${type==="receipt"?"var(--green)":"var(--red)"};">${type==="receipt"?"+":"−"}${fmA(it.amount,it.currency)}</div></div></div>`;
    }
    return "";
  }).join("")||`<div class="empty"><p>${t("noRecords")}</p></div>`;
  
  modal(title,`
    <div style="background:linear-gradient(135deg,var(--accent-dim),var(--purple-dim));padding:16px;border-radius:14px;margin-bottom:14px;text-align:center;">
      <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">${totalLabel}</div>
      <div style="font-size:26px;font-family:var(--mono);font-weight:300;color:var(--accent);">${fmA(total)}</div>
      <div style="font-size:12px;color:var(--text3);margin-top:4px;">${items.length} records</div>
    </div>
    <div class="card">${list}</div>`,
    `<button class="btn btn-secondary" onclick="closeModal()">${t("close")}</button>`,true);
}
...
async function saveBud(){
  ...
  S.budgets.unshift(d);closeModal();toast(t("saved"),"ok");render("budgets");
}
...
async function saveGoal(){
  ...
  S.goals.unshift(d);closeModal();toast(t("saved"),"ok");render("goals");
}
